...
const KATEGORIE_Z_BDO = [
    ...
    { nazwa: "Karton / Papier", bdo: "15 01 01" },
...
const SLOWA_KLUCZE: { slowa: string[], kategoria: string }[] = [
    ...
    { slowa: ["karton","kartony","tektura","makulatura","gazeta","gazety","papier mix"], kategoria: "Karton / Papier" },
...
const JEDNOSTKI_MAPA: { wzorce: string[], skrot: 'kg' | 't' | 'szt' }[] = [
    { skrot: 'kg', wzorce: ['kilogram','kilogramy','kilogramow','kilo','kg'] },
    { skrot: 't',  wzorce: ['tona','tony','ton','tone','tonn','tonne',' t ',' t,',' t.','/t'] },
    { skrot: 'szt', wzorce: ['sztuka','sztuki','sztuk','szt','piece','pieces','calosciowo','calosc','komplet'] },
];
...
// ─── Kody krajów → długość numeru lokalnego ───────────────────────────────
const KODY_KRAJOW: Record<string, number> = {
    '1':10,'7':10,'20':10,'27':9,'30':10,'31':9,'32':9,'33':9,'34':9,
    '36':9,'39':10,'40':9,'41':9,'43':10,'44':10,'45':8,'46':9,'47':8,
    '48':9,'49':11,'51':9,'52':10,'55':11,'61':9,'64':9,'81':10,'82':10,
    '86':11,'91':10,'380':9,'420':9,'421':9,
};

/**
 * Formatuje numer telefonu na bieżąco.
 *
 * Obsługuje:
 *   Polski (9 cyfr):   "600700800"  → "600 700 800"
 *   +XX format:        "+44700..."  → "+44 700 900 123"
 *   00XX format:       "004470..."  → "0044 700 900 123"
 *
 * Logika:
 * - Pozwala wpisywać + tylko na początku
 * - Cyfry grupuje inteligentnie: kod kraju wyznacza z tabeli KODY_KRAJOW
 * - Nie obcina cyfr — użytkownik może wpisać dowolnie długi numer zagraniczny
 * - Nie blokuje wpisywania spacji/myślników (przepuszcza je)
 */
function formatujTelefon(raw: string): string {
    if (!raw) return '';

    const trimmed = raw.trimStart();
    const maPlus  = trimmed.startsWith('+');
    const ma00    = !maPlus && trimmed.startsWith('00');

    // Wyciągnij samo cyfry (bez + i bez spacji)
    const cyfry = trimmed.replace(/\D/g, '');
    if (!cyfry) return maPlus ? '+' : '';

    // ── NUMER ZAGRANICZNY ──────────────────────────────────────────────────
    if (maPlus || ma00) {
        const cyfryBezPrefiks = ma00 ? cyfry.slice(2) : cyfry;

        // Wyznacz kod kraju (1-, 2- lub 3-cyfrowy)
        let dlKodu = 2; // domyślnie 2
        if (KODY_KRAJOW[cyfryBezPrefiks.slice(0, 3)] !== undefined) dlKodu = 3;
        else if (KODY_KRAJOW[cyfryBezPrefiks.slice(0, 1)] !== undefined &&
                 KODY_KRAJOW[cyfryBezPrefiks.slice(0, 2)] === undefined) dlKodu = 1;

        const kod   = cyfryBezPrefiks.slice(0, dlKodu);
        const numer = cyfryBezPrefiks.slice(dlKodu);          // cyfry numeru lokalnego
        const grupy = numer.match(/.{1,3}/g) || [];           // grupy po 3

        const prefix = maPlus ? `+${kod}` : `00${kod}`;
        return numer ? `${prefix} ${grupy.join(' ')}` : prefix;
    }

    // ── NUMER KRAJOWY (9 cyfr, format XXX XXX XXX) ─────────────────────────
    const tylko9 = cyfry.slice(0, 9);
...
interface ParsedData {
    telefon?: string; waga?: string; jednostka?: 't'|'kg'|'szt';
    cena?: number|null; miejscowosc?: string; wojewodztwo?: string;
    material?: string; autoBdo?: string; title?: string;
    typOferty?: 'sprzedam'|'kupie'; website_url?: string; supplyFreq?: SupplyFreq;
}
...
function parsujTekst(tekst: string): ParsedData {
    ...
    if (!negFrazy.some(f=>t.includes(f))) { const cenaMatch=tekst.match(/(\d+[\.,]?\d*)\s*(?:zł|pln|zl)/i); if (cenaMatch) wynik.cena=parseFloat(cenaMatch[1].replace(',','.')); }
    ...
    if (t.includes('cala polska')||t.includes('caly kraj')||t.includes('ogolnopolski')) { wynik.miejscowosc='Cała Polska'; wynik.wojewodztwo=''; }
        ...
        for (const [k,v] of Object.entries(WOJEWODZTWA_ASCII)) { if (t.includes(k)) { wynik.wojewodztwo=v; break; } }
        for (const [k,wk] of Object.entries(MIASTA_WOJEWODZTWA)) { if (t.includes(k)) { wynik.miejscowosc=MIASTA_NAZWY_PL[k]||k; if (!wynik.wojewodztwo) wynik.wojewodztwo=WOJEWODZTWA_ASCII[wk]||wk; break; } }
    }
    ...
    const ZBEDNE=['sprzedam','kupie','oferuje','oferujemy','zapraszamy','oddam','firma','przedsiebiorstwo','spolka','oferta','ogloszenie','tel','telefon','kontakt','dzwon','ilosc','ilo','cena','cene'];
    const tbezUrl=tekst.replace(URL_REGEX,'').replace(/\s+/g,' ').trim();
    const sMatch=tbezUrl.match(/(regranulat|re\s*granulat|przemial|aglomerat|folia|zlom|makulatura|karton|drewno|kabel|platki|butelk|kanister|recyklat|aglo)[\s\w\-\/]{0,50}/i);
    if (sMatch) { let tyt=sMatch[0].replace(/[\.,!?;:]+$/,'').replace(/(?:tel\.?|telefon|kontakt|dzwon|ilo[sś][cć]?|\d{9})[^\w].*$/i,'').trim(); const tN=norm(tyt); for (const f of ZBEDNE) { if (tN.includes(f)) tyt=tyt.replace(new RegExp('(?:^|\\s)'+f+'(?:\\s|$)','gi'),' '); } tyt=tyt.replace(/\s+/g,' ').trim().substring(0,55); if (tyt.length>3) wynik.title=tyt.charAt(0).toUpperCase()+tyt.slice(1); }
    if (!wynik.title||wynik.title.length<4) { let l=tbezUrl.split('\n')[0].trim(); l=l.replace(/\b(?:z|ze)\s+\w+(?:a|y|i|u|ów|em)\b/gi,''); const lN=norm(l); for (const f of ZBEDNE) { if (lN.includes(f)) l=l.replace(new RegExp('(?:^|\\s)'+f+'(?:\\s|$)','gi'),' '); } l=l.replace(/\s+/g,' ').trim(); if (l.length>3) wynik.title=l.charAt(0).toUpperCase()+l.slice(1).substring(0,55); }
    return wynik;
}
...
export default function DodajOferteKrok1() {
    ...
    const startMikrofon = () => {
        ...
        if (!SR) return;
        const r=new SR(); r.lang='pl-PL'; r.continuous=false; r.interimResults=false; r.maxAlternatives=1;
        recognitionRef.current=r;
        r.onstart=()=>setNasluchuje(true);
        r.onresult=(e:any)=>{ const t=e.results[0]?.[0]?.transcript||''; if (t) setMagicTekst(p=>p?p+' '+t:t); };
        r.onend=()=>setNasluchuje(false);
        r.onerror=(e:any)=>{ setNasluchuje(false); if (e.error==='not-allowed') alert('Zezwól na dostęp do mikrofonu.'); };
        try { r.start(); } catch { setNasluchuje(false); }
    };
    ...
    const handleAnalizuj = () => {
        ...
        const zastap=(pole:string,nv:string,ov:string,setter:(v:string)=>void)=>{
            if (!nv) return;
            if (ov&&ov!==nv) { if (!confirm(`Pole "${pole}" ma już "${ov}". Zastąpić?`)) return; }
            setter(nv);
        };
        ...
        if (parsed.cena!=null) localStorage.setItem('magic_cena',String(parsed.cena));
        ...
        setPodswietlone(nowe);
        const nt=parsed.title||title; const nm=parsed.material||material;
        const nl=[parsed.miejscowosc||miejscowosc,parsed.wojewodztwo||wojewodztwo].filter(Boolean).join(', ');
        const jl=parsed.jednostka==='kg'?'kg':parsed.jednostka==='szt'?'szt.':'t';
        const opis=[nt||nm,nm&&nm!==nt?`Kategoria: ${nm}`:'', (parsed.waga||waga)?`Ilość: ${parsed.waga||waga} ${jl}`:'', nl?`Lokalizacja: ${nl}`:''].filter(Boolean).join('\n');
        setSeoOpis(opis); localStorage.setItem('magic_opis',opis);
        const slug=[nt||nm,parsed.miejscowosc||miejscowosc].filter(Boolean).join(' ');
        if (slug) localStorage.setItem('magic_slug',slugify(slug));
        setSeoWygenerowane(true);
    };

    const gfc=(pole:string,base='')=>podswietlone.has(pole)?`${base} border-yellow-400 bg-yellow-50`:`${base} bg-slate-50 border-slate-200`;

    const uploadImage=async(f:File)=>{
        const opts={maxSizeMB:1,maxWidthOrHeight:1080,useWebWorker:true};
        const c=await imageCompression(f,opts);
        const fn=`${Date.now()}-${Math.random().toString(36).substring(2,9)}.png`;
        await supabase.storage.from('oferty-zdjecia').upload(fn,c);
        const {data}=supabase.storage.from('oferty-zdjecia').getPublicUrl(fn);
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 sm:p-8 font-sans">
            <div className="max-w-xl w-full bg-white rounded-[40px] shadow-2xl p-8 md:p-12 border-4 border-white">
                ...
                {!magicOtwarte && (
                    <button type="button" onClick={handleHeroClick} className="w-full mb-6 group relative overflow-hidden rounded-[28px] bg-gradient-to-br from-slate-900 to-blue-900 p-6 text-left transition-all hover:scale-[1.02] active:scale-[0.98] shadow-2xl">
                        ...
                        <div className="relative flex items-center gap-5">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-slate-300 text-xs font-medium leading-relaxed">{jestIOS?'Kliknij → dotknij pola → naciśnij 🎤 na klawiaturze → dyktuj.':'Powiedz co masz, ilość, miasto i telefon — AI wypełni resztę.'}</p>
                            </div>
                    ...
                    <div className="border-2 border-blue-400 rounded-[28px] overflow-hidden shadow-lg mb-6">
                        <div className="bg-blue-600 px-5 py-3 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                {wspieraMikrofon&&!jestIOS&&(
                                    <button type="button" onClick={startMikrofon} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${nasluchuje?'bg-red-500 text-white animate-pulse':'bg-white/20 text-white hover:bg-white/30'}`}>
                        ...
                        <div className="p-4 bg-white">
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                                {[{label:'Tony...',tekst:'Sprzedam 24 tony regranulatu PP czarny, cena 2500 zł/t, tel. 600 700 800, śląskie.'},{label:'Kilogramy...',tekst:'Sprzedam 500 kg kabla miedzianego, cena 30 zł/kg. Tel. 500 100 200, Katowice.'},{label:'Sztuki...',tekst:'Oddam 50 szt. palet drewnianych, odbiór własny. Tel. 400 300 200. Kraków.'}].map((s,i)=>(
                ...
                <form onSubmit={handleDalej} className="space-y-6">
                    ...
                    <div>
                        ...
                        <input required type="text" placeholder="np. Regranulat LDPE jasny" className={`w-full p-5 border-2 rounded-[24px] font-bold focus:border-blue-500 outline-none transition-colors ${gfc('title')}`} value={title} onChange={e=>{setTitle(e.target.value);setPodswietlone(p=>{const n=new Set(p);n.delete('title');return n;});}}/>
                    </div>
                    ...
                    <div>
                        ...
                        <select required className={`w-full p-5 border-2 rounded-[24px] font-bold outline-none focus:border-blue-500 transition-colors ${gfc('material')}`} value={material} onChange={e=>{setMaterial(e.target.value);const f=KATEGORIE_Z_BDO.find(k=>k.nazwa===e.target.value);if (f) setAutoBdo(f.bdo);setPodswietlone(p=>{const n=new Set(p);n.delete('material');return n;});}}>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" placeholder="np. 24" className={`w-full p-5 border-2 rounded-[24px] font-bold outline-none focus:border-blue-500 transition-colors ${gfc('waga')}`} value={waga} onChange={e=>{setWaga(e.target.value);setPodswietlone(p=>{const n=new Set(p);n.delete('waga');return n;});}}/>
                        </div>
                        <div>
                            <label className="text-[10px] font-black uppercase text-slate-400 ml-5 mb-1 block">
                                Telefon <span className="text-red-500">*</span>
                            </label>
                            {/* ── POLE TELEFONU z pełnym formatowaniem ── */}
                            <input
                                required
                                type="tel"
                                inputMode="tel"
                                ...
                                className={`w-full p-5 border-2 rounded-[24px] font-bold outline-none focus:border-blue-500 transition-colors tracking-wider ${gfc('telefon')}`}
                                value={telefon}
                                onChange={e => {
                            ...
                            <p className="text-[10px] text-slate-400 font-bold ml-5 mt-1">
                                Zagraniczny: +44 700 900 123 lub 0044...
                            </p>
                        </div>
                    </div>
                    ...
                    {!jestZasiegOgolny&&(
                        <div>
                            ...
                            <input type="text" placeholder="np. Toruń, Bydgoszcz..." className={`w-full p-5 border-2 rounded-[24px] font-bold outline-none focus:border-blue-500 transition-colors ${gfc('miejscowosc')}`} value={miejscowosc} onChange={e=>{setMiejscowosc(e.target.value);setPodswietlone(p=>{const n=new Set(p);n.delete('miejscowosc');return n;});}}/>
                            <p className="text-[10px] text-slate-400 font-bold ml-5 mt-1">Opcjonalne — pomaga kupującym z sąsiedztwa</p>
                        </div>
                    )}

                    <div>
                        <label className="text-[10px] font-black uppercase text-slate-400 ml-5 mb-2 flex items-center gap-2"><Calendar size={12}/> Jak często :</label>
                        <div className="grid grid-cols-2 gap-2">
                            {([{v:'jednorazowo',label:'Tylko raz'},{v:'co_tydzien',label:'Co tydzień'},{v:'co_miesiac',label:'Co miesiąc'},{v:'stala_wspolpraca',label:'Stała współpraca'}] as const).map(opt=>(
                                <button key={opt.v} type="button" onClick={()=>setSupplyFreq(prev => prev===opt.v ? null : opt.v)} className={`flex items-center justify-center p-3 rounded-2xl border-2 font-black text-xs uppercase tracking-widest transition-all ${supplyFreq===opt.v?'bg-emerald-600 border-emerald-600 text-white shadow-md':'bg-slate-50 border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                                    {opt.label}
                                </button>
                            ))}
                        </div>
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white py-8 rounded-[32px] font-black text-2xl uppercase flex items-center justify-center gap-4 hover:bg-blue-600 transition-all shadow-xl active:scale-95 disabled:opacity-50 mt-4">
                        {loading?'Przetwarzanie...':editMode?'Zapisz zmiany':'Ostatni krok'}
                        <CheckCircle size={28}/>
                    </button>
                </form>
            </div>
