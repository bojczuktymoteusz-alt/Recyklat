                    ...
                    <div ref={kategorieRef} className="bg-white p-2 rounded-3xl shadow-xl border border-gray-100 flex gap-2 overflow-x-auto scrolling-touch pr-4" style={{ scrollSnapType: 'none' }}>
                        {KATEGORIE.map(kat => (
                            <button key={kat.nazwa} onClick={() => setAktywnyFiltr(kat.nazwa)}
                                className={`flex-shrink-0 min-w-[42%] sm:min-w-[auto] flex items-center gap-2 px-6 py-3 rounded-2xl transition-all font-black text-xs uppercase tracking-widest ${aktywnyFiltr === kat.nazwa ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-50'}`}>
